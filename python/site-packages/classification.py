...
def create_dataset(mode, is_train, **kwargs):
    ...
    if mode in ['cifar10', 'cifar100']:
        ...
        albu = A.Compose([
            ...
            A.Rotate(limit=5),
            A.RandomResizedCrop((32, 32), scale=(0.8, 1.25)),
            # A.ShiftScaleRotate(
...
class TrainerConfig(BaseTrainerConfig):
    ...
    dataset:str = Field('number', choices=['number', 'fashion', 'cifar10', 'cifar100'], s='-d')
...
class Trainer(BaseTrainer):
    ...
    def prepare(self):
        ...
        if self.config.graph:
            graph_matrix = GraphMatrix(size=num_classes)
    ...
    def eval(self, inputs, gts, i):
        ...
        if self.config.graph:
            ...
            if i == 0:
                ...
                print(self.model.graph_matrix.get_matrix().softmax(dim=0))
...
class CLI(BaseMLCLI):
    ...
    def run_cluster(self, a:TrainArgs):
        ...
        for i in tqdm(range(a.limit)):
            ...
            x = x[None, ...]
            with torch.set_gosrad_enabled(False):
        ...
        for gt in np.unique(gts):
            ...
            for t in [True, False]:
                ...
                scatters.append(plt.scatter(xx[idx], yy[idx],
                                            marker='o'if t else 'x', s=10, label=gt, c=f'C{gt}'))
