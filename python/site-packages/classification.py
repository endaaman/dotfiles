...
def create_mnist_dataset(mode, is_train, aug, **kwargs):
    if mode == 'number':
        ds = datasets.MNIST(
            './datasets/MNIST',
            train = is_train,
        ...
        if aug:
            aug = lambda x: albu(image=x.squeeze())['image']
        else:
            aug = lambda x: torch.from_numpy(x/255).to(torch.float32).permute(2, 0, 1)
...
class ClsDatgaset(Dataset):
    def __init__(self, dataset:str, is_train:bool, aug:bool):
        ...
        self.aug = aug
        self.data, self.labels, self.aug = create_mnist_dataset(dataset, is_train, aug)
...
class Trainer(BaseTrainer):
    ...
    def create_scheduler(self):
        ...
        # return optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=20, eta_min=self.config.lr/10)
