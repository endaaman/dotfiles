...
from endaaman.ml import BaseMLCLI, BaseDLArgs, BaseTrainer, BaseTrainerConfig, tensor_to_pil, pil_to_tensor
...
class IICModel(nn.Module):
    def __init__(self, input_size=1, num_cluster=10, num_over_cluster=100):
        ...
        self.base = timm.create_model('resnet18', in_chans=input_size)
...
class Trainer(BaseTrainer):
    ...
    def eval(self, inputs, gts):
        ...
        data1, data1_over = self.model(inputs.to(self.device), activate=True)
        data2, data2_over = self.model(pairs.to(self.device), activate=True)

        loss = iic_loss(data1, data2)
        loss_over = iic_loss(data1_over, data2_over)
...
class CLI(BaseMLCLI):
    ...
    def run_pred(self, a):
        ...
        for gt in np.unique(gts):
            needle = gts == gt
            plt.scatter(embedding_x[needle], embedding_y[needle], s=24, label=gt)
