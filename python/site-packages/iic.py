    ...
    def __init__(self, dataset:str, is_train:bool, dual:bool):
        ...
        if dataset == 'number':
            ...
            self.aug = A.Compose([
                ...
                A.ElasticTransform(p=1, alpha=1, sigma=3, alpha_affine=5),
            ])
        else:
...
class IICLoss(nn.Module):
    ...
    def forward(self, p, q):
        ...
        # Pm, Qm: mean terms
        Pm = m.mean(dim=0).view(k, 1).expand(k, k)
        Qm = Pm.t()
        # Pm = J.sum(dim=1).view(k, 1).expand(k, k)
        # Qm = J.sum(dim=0).view(1, k).expand(k, k)

        # Ps, Qs: std terms
        Ps = m.std(dim=0).view(k, 1).expand(k, k)
        Qs = Ps.t()

        a = self.alpha
        b = self.beta
        loss = J * (a*Pm.log() + a*Qm.log() - b*Ps.log() - b*Qs.log() - J.log() )
        loss = loss.sum()
        return loss
