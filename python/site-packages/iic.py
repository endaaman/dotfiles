...
def iic_loss(i, j):
    ...
    alpha = 2.0
    loss = p_i_j * (alpha*torch.log(p_j) + alpha*torch.log(p_i) - torch.log(p_i_j))
    loss = loss.sum()

    return loss


def pc_loss(xx, yy, sames):
    EPS = sys.float_info.epsilon
    losses = []
...
class PairDatgaset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        z = self.aug(image=x)['image']
        y = torch.tensor(y[None, :, :]/255).to(torch.float)
        z = torch.tensor(z[None, :, :]/255).to(torch.float)
        return y, z
...
class Trainer(BaseTrainer):
    ...
    def eval(self, xx, yy):
        ...
        loss = iic_loss(x_pred, y_pred)
        loss_over = iic_loss(x_pred_over, y_pred_over)
...
class CLI(BaseMLCLI):
    ...
    def run_train(self, a:TrainArgs):
        ...
        t = Trainer(
            loss_fmt='{:.10f}',
            config=config,
            out_dir=f'out/models/{self.with_suffix(a.name, a.suffix)}',
            train_dataset=dss[0],
            val_dataset=dss[1],
