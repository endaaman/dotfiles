    ...
    if mode == 'cifar10':
        ...
        albu = A.Compose(pack([
            ...
            # A.OneOf([
                ...
                # ], p=1.0),
            A.Normalize(**norm_args) if normalize else None,
            ToTensorV2(),
            ]))
        aug_train = lambda x: albu(image=x.squeeze())['image']
        aug_test = transforms.Compose(pack([
            transforms.ToTensor(),
            transforms.Normalize(**norm_args) if normalize else None,
            ]))
        return data, labels, aug_train, aug_test
...
class CLI(BaseMLCLI):
    ...
    def run_cluster(self, a):
        ...
        for i in tqdm(range(a.limit)):
            ...
            with torch.no_grad():
                pred, y_o = model(x[None, ...], activate=True)
            # pred = y_o
            p = torch.argmax(pred)
        ...
        for gt in np.unique(gts):
            ...
            ii = [cv2.resize(i[0], (140, 140)) for i in images[needle]]
            iis.append(ii)
