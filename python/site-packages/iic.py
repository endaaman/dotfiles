...
class IICDatgaset(Dataset):
    def __init__(self, dataset:str, size:int, is_train:bool, dual:bool):
        self.dataset = dataset
        self.dual = dual
        if dataset == 'number':
            self.ds = create_mnist_dataset(dataset, is_train)
            self.aug = A.Compose([
                # A.Resize(size, size),
                A.ShiftScaleRotate(
            ...
            self.aug = A.Compose([
                # A.Resize(size, size),
                A.ShiftScaleRotate(
                    shift_limit=(-0.3, 0.3), scale_limit=(-0.1, 0.2),
                    rotate_limit=(-10, 10), p=1.0
                ),
                A.OneOf([
...
class CLI(BaseMLCLI):
    ...
    class TrainArgs(CommonArgs):
        ...
        dataset:str = Field('number', choices=['number', 'fashion'], s='-d')

    def run_train(self, a:TrainArgs):
        dss = [
            IICDatgaset(dataset=a.dataset, size=a.size, is_train='train'==t, dual=a.dual)
            for t in [True, False]
        ]
