...
class BaseTrainerConfig(BaseModel):
    ...
    @classmethod
    def from_file(cls: type[T], config_path) -> T:
...
class Checkpoint(NamedTuple):
    ...
    val_history: dict = None
    random_states: dict = None
    additional_data: dict = None

    @classmethod
    def from_file(cls, chp_path):
        d = torch.load(chp_path, map_location=torch.device('cpu'))
...
class BaseTrainer(metaclass=ABCMeta):
    def __init__(self,
        ...
        self.multi_gpu = multi_gpu and (not re.match(r'.*:\d$', device)) and torch.cuda.device_count() > 1
    ...
    def select_out_dir(self, out_dir):
        ...
        while True:
            ...
            # Acquire inherited class type and cast then get dict
            # to fill default value for the old version
            C = self.config.__class__
            old_config = C(**old_config).dict()
    ...
    def create_optimizer(self):
        ...
        # return optim.SGD(self.model.parameters(), lr=self.config.lr, momentum=0.9)
    ...
    def save_checkpoint(self, path):
        with EvacuateModel(self.model, self.device) as model:
    ...
    def step(self):
        ...
        if isinstance(self.scheduler, optim.lr_scheduler.ReduceLROnPlateau):
            ...
            history = self.val_history if self.val_loader else self.train_history
    ...
    def _eval(self, params, is_training, iter_index):
        with torch.set_grad_enabled(is_training):
            loss, preds = self.eval(*params, iter_index)
        if is_training:
    ...
    def calc_metrics_and_get_message(self, preds, gts, history=None):
        ...
        for key, m in self.metrics.items():
            if preds is None or gts is None:
                continue
            vv = m(preds, gts, history is None)
            if vv is None:
                continue
    ...
    def evaluate_loader(self, is_training, loader, history):
        ...
        losses = []
        t = tqdm(enumerate(loader), leave=False, dynamic_ncols=True, total=len(loader))
        predss = []
        gtss = []
        for i, params in t:
            inputs, gts = self.select_inputs_and_gts(params)
            loss, preds, = self._eval(params, is_training, i)
            loss_value = float(loss.item())
            losses.append(loss_value)
            batch_metrics_message = self.calc_metrics_and_get_message(preds, gts, None)
            loss_str = self.loss_fmt.format(loss_value)
            t.set_description(f'[{target}] loss:{loss_str} ' + batch_metrics_message)
            t.refresh()
            predss.append(preds)
            gtss.append(gts)
            # self.record_value(f'loss_iter/{target}', loss_value)
    ...
    def plot(self, train_preds, train_gts, val_preds, val_gts):
        mp_backend = mpl.rcParams['backend']
        mpl.use('Agg')
        ...
        figsize = self.fig_size or (2+min(col_count, 2) * 2, 2 * row_count) # 1:6, 2:6, 3:8
    ...
    def save_predictions(self, train_preds, train_gts, val_preds, val_gts):
        ...
        if self.save_best and self.is_achieved_best():
    ...
    def start(self, total_epoch):
        ...
        while self.current_epoch <= total_epoch:
            ...
            print(f'[{self.current_epoch}/{total_epoch}] Start training (lr={lr}) ({now})')
