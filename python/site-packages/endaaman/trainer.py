...
class BaseTrainer(metaclass=ABCMeta):
    ...
    def select_out_dir(self, out_dir):
        ...
        while True:
            ...
            print(f'The old config {config_file_path} already exists but is different with the current config. \n' \
                  f'[current]:\n{self.config}\n' \
                  f'[old] :\n{old_config}\n')
    ...
    def save_predictions(self, train_preds, train_gts, val_preds, val_gts):
        ...
        vv = [v.detach().cpu() for v in (train_preds, train_gts, val_preds, val_gts)]
        torch.save(dict(zip(names, vv)), J(self.out_dir, PREDICTIONS_FILE))
