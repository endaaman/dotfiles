...
class MILDataset(Dataset):
    ...
    def get_label_by_global_idx(self, global_idx):
        ...
        for label_index, T in enumerate(self.counts_by_label):
            ...
            S += T
        raise RuntimeError(f'{global_idx} maybe out of range ({S})')
...
class CLI(BaseMLCLI):
    ...
    def run_pred(self, a:PredArgs):
        ...
        fig, axes = plt.subplots(2, 1, figsize=(5,5), tight_layout=True, facecolor="whitesmoke")
        ...
        pred = ' '.join([f'{config.target_labels[i]}:{p:.3f}' for i, p in enumerate(y)])
        plt.suptitle(f'pred: {pred}')
