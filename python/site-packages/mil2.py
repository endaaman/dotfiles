...
class SpreadAttentionModel(nn.Module):
    def __init__(self, activation='softmax', params_count=64, num_classes=1):
        super().__init__()
        self.activation = activation
        self.params_count = params_count
...
class MAMILModel(nn.Module):
    def __init__(self, activation='softmax', params_count=64, num_classes=1):
        ...
        self.mse_w = nn.Linear(self.num_features, self.params_count, bias=False)
        self.mse_b = nn.Parameter(torch.Tensor(np.zeros(self.params_count)))

        self.feature_linear = nn.Linear(self.num_features, self.params_count)

        # to attention
        self.integrated_linear = nn.Linear(self.params_count*2, 1)
    ...
    def compute_attentions(self, features):
        ...
        feature = self.feature_linear(features)

        integrated = torch.cat([mse, feature], dim=1)

        attentions = self.integrated_linear(integrated)
    ...
    def forward(self, x, activate=False, with_attentions=False):
        ...
        print(yy.shape)
        print(aa.shape)
        y = (yy * aa.view(-1, self.num_classes)).sum(dim=0)
