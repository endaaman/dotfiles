...
class BaseAttentioModel(nn.Module):
...
class MILModel(BaseAttentioModel):
...
class CrossMILModel(BaseAttentioModel):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        self.integrated_u = nn.Linear(self.params_count*2, 1)
    ...
    def compute_attentions(self, features):
        ...
        aa = self.integrated_u(integrated)
...
class ResMILModel(BaseAttentioModel):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def compute_attentions(self, features):
        ...
        for feature, pre_w in zip(features, pre_ww):
            ...
            m = m.sum(dim=-2)
            m = torch.concat([feature, m])
            aa.append(self.compute_attention_scores(m))
        aa = torch.stack(aa).flatten()
        return aa
