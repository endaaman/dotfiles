...
from .torch_utils import restore_random_states, get_random_states
...
class Trainer(metaclass=ABCMeta):
    ...
    def get_out_dir(self):
        return os.path.join('out', self.trainer_name)
    ...
    def create_scheduler(self, lr, max_epoch):
        ...
        return optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lambda _: lr)
    ...
    def prepare_mlops(self):
        if not self.writer:
            tb_log_dir = os.path.join(*list(filter(None, [
                self.log_dir,
                'tensorboard',
        ...
        if mlflow.active_run() is None:
            ...
            if experiment := mlflow.get_experiment_by_name(name=name):
                experiment_id = experiment.experiment_id
            else:
                experiment_id = mlflow.create_experiment(name=name)
            experiment_id = mlflow.create_experiment(name=self.trainer_name, tags={})
            mlflow.start_run(experiment_id=experiment_id)

        print('using log dir: mlflow=`{mf_log_dir}` tensorboard=`{tb_log_dir}`')
    ...
    def start(self, max_epoch, lr=0.0001, checkpoint=None):
        ...
        self.scheduler = self.create_scheduler(lr, max_epoch)
        ...
        for epoch in range(initial_epoch, max_epoch + 1):
                ...
                with torch.set_grad_enabled(False):
                    ...
                    for i, (inputs, labels) in t:
                        ...
                        if self.report_image:
                            images += self.eval_image(inputs, labels, outputs)
                ...
                for group_name, group in groups.items():
                    if group_name:
                        ...
                        for k in group:
                            ...
                            mlflow.log_metric(f'{k}/{target}', history[k][-1])
            ...
            #* save checkpoints
            if (epoch % self.save_period == 0) or (self.save_period < 0 and epoch == max_epoch):
