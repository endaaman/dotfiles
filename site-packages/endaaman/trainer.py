...
from .torch import get_random_states, restore_random_states, TorchCommander
...
class Trainer(metaclass=ABCMeta):
    def __init__(self, name, model, loaders, log_dir='data/logs', device='cpu', save_period=10, show_fig=False, suffix='', **kwargs):
        self.name = name
        self.model = model
    ...
    def require_writer(self):
        if not self.writer:
            self.writer = SummaryWriter(log_dir=os.path.join(self.log_dir, self.with_suffix(self.name)))
...
class TrainCommander(TorchCommander):
    ...
    def _arg_common(self, parser):
        ...
        parser.add_argument('--save-period', type=int, default=D('save_period', 10))
    ...
    def start(self, trainer, max_epoch=None, lr=None, checkpoint=None):
        max_epoch = max_epoch or self.a.epoch
        lr = lr or self.a.lr
        trainer.start(max_epoch, lr, checkpoint)
