...
class BaseTrainer(metaclass=ABCMeta):
    def __init__(self,
        ...
        # select out dir
        chp_path = self.select_out_dir(out_dir, overwrite)
        if chp_path:
    ...
    def select_out_dir(self, out_dir, overwrite):
        # returns needs to restore checkpoint
        if not os.path.isdir(out_dir) or overwrite:
        ...
        with pd.ExcelWriter(path, engine='xlsxwriter') as writer:
            for t, history in hh.items():
                ...
                for i, k in enumerate(history.keys()):
                    fmt = self.loss_fmt if k == 'loss' else self.metrics[k].format
                    base = fmt.format(.0)
