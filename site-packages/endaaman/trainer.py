...
class BaseTrainer(metaclass=ABCMeta):
    ...
    def select_out_dir(self, out_dir):
        ...
        if old_config == self.config:
            self.out_dir = out_dir
            chp_path = J(self.out_dir, LAST_CHECKPOINT_FILE)
    ...
    def get_format_by_key(self, key):
        if key == 'loss':
            return self.loss_fmt
        return getattr(self.metrics[key], 'format', DEFAULT_FORMAT)
    ...
    def plot(self, train_preds, train_gts, val_preds, val_gts):
        ...
        figsize = self.fig_size or (B+min(count, (B-1)) * (B-1), B * row_count) # 1:6, 2:8, 3:10
        ...
        for k, vis in self.visualizers.items():
            ...
            x = i % col_count
            ax = axes[y, x] if row_count > 1 else axes[x]
            vis(ax, train_preds, train_gts, val_preds, val_gts)
            i += 1
    ...
    def start(self, total_epoch):
        ...
        while self.current_epoch <= total_epoch:
            ...
            train_preds, train_gts = self.eval_loader(True, self.train_loader, self.train_history)

            if self.val_loader:
                with torch.set_grad_enabled(False):
                    val_preds, val_gts = self.eval_loader(False, self.val_loader, self.val_history)
            else:
                val_preds = None
                val_gts = None
