...
class Commander():
    ...
    def run(self):
        ...
        if len(args_names) == 0:
            ...
            maxlen = max(0, *[len(k) for k in args_names])
            items = [
        ...
        if function_as_param in self.options.get('no_pre_common', []):
            print('Skip executing pre_common()')
        else:
            self._pre_common()
    ...
    def exec(self, func, params=None):
        params = params or []
        with patch('sys.argv', [func] + params):
            self.run()
