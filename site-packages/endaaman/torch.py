...
class Trainer(TorchCommander):
    ...
    def prepare(self, model, optimizer=None, scheduler=None):
        self.prepared = True
        self.model = model
        self.optimizer = optimizer or optim.Adam(self.model.parameters(), lr=self.args.lr)


    def train_model(self, name, train_loader, val_loader, eval_fn,
                    batch_metrics=None, epoch_metrics=None, short=False):
        if not self.prepared:
            raise RuntimeError('Need prepare()')
        ...
        for epoch in range(1, self.args.epoch + 1):
            ...
            if epoch > 1:
                ...
                if epoch == 2 and not self.args.no_show_fig:
                    subprocess.Popen(['/usr/bin/xdg-open', fig_path])
                fig.clf()
                plt.clf()
                plt.close()
            ...
            if epoch % self.args.period_save_weight == 0:
                weights_path = self.save_state(name, self.model, epoch, train_history, val_history)
                print(f'{header}Saved "{weights_path}"')


            if isinstance(self.scheduler, optim.lr_scheduler.ReduceLROnPlateau):
                self.scheduler.step(train_metrics['loss'][-1])
            else:
                self.scheduler.step()
            print()
