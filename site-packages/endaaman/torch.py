...
class Trainer(Commander):
    def _arg_common(self, parser):
        parser.add_argument('--lr', type=float, default=self.defaults.get('lr', 0.01))
        parser.add_argument('-e', '--epoch', type=int, default=self.defaults.get('epoch', 30))
        parser.add_argument('-f', '--first-record-epoch', type=int, default=0)
        parser.add_argument('-p', '--period-save-weight', type=int, default=10)
        parser.add_argument('-n', '--no-show-fig', action='store_true')
        super()._arg_common(parser)

    def save_state(self, name, model, epoch, train_history, val_history):
        state = {
            'name': name,
        ...
        weights_path = os.path.join(out_dir, f'{epoch}.pth')
