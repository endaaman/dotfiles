...
from .torch_utils import get_random_states, restore_random_states, fix_random_states, get_global_seed, set_global_seed
from .commander import Commander
from .trainer import Trainer, Checkpoint
from .predictor import Predictor
from .torch_utils import pil_to_tensor, tensor_to_pil
...
TrainerClassDef = TypeVar('TrainerClassDef', bound=Trainer)



def get_torch_args(D):
    class TorchArgs(BaseModel):
        seed: int = Field(D.get('seed', get_global_seed()), cli=('--seed', ))
        suffix: str = Field('', cli=('--suffix', ))

        cpu: bool = Field(False, cli=('--cpu', ))
        batch_size: int = Field(D.get('batch_size', 8), cli=('--batch-size', '-B'))
        num_workers: int = Field(D.get('num_workers', 4), cli=('--num_workers', '-N'))
        lr: float = Field(D.get('lr'), cli=('--lr', ))
        epoch: int = Field(D.get('epoch', 50), cli=('--epoch', '-E'))
        log_dir: str = Field('runs', cli=('--log-dir', ))
        save_period: int = Field(-1, cli=('--save', ))

        def __init__(self, **data):
            super().__init__(**data)
            fix_random_states(self.seed)
        ...
        def create_trainer(self, TrainerClass:TrainerClassDef, model_name, loaders, trainer_name=None, experiment_name=None, extra=None):
            return TrainerClass(
                model_name,
                loaders,
                experiment_name=experiment_name,
