...
class TrainCommander(TorchCommander):
    ...
    def save_state(self, name, state_dict, epoch, train_history, val_history, data=None):
        state = {
            'name': name,
            'epoch': epoch,
    ...
    def train_model(self, name, model, loaders, eval_fn,
                    batch_metrics=None, epoch_metrics=None,
                    optimizer=None, scheduler_fn=None, state_dict_fn=None,
                    no_metrics=False):
        ...
        for epoch in range(1, self.args.epoch + 1):
            ...
            if epoch % self.args.save_period == 0:
                state_dict = state_dict_fn(model) if state_dict_fn else get_state_dict(model)
                weights_path = self.save_state(name, state_dict, epoch, train_history, val_history)
