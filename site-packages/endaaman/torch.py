...
from .torch_utils import get_random_states, restore_random_states, fix_global_seed, get_global_seed
...
class TorchCLI(MLCLI):
    def _pre_common(self, a):
        self.use_gpu = not self.a.cpu and torch.cuda.is_available()
        self.use_multi_gpu = not self.using_gpu() and torch.cuda.device_count() > 1
    ...
    def create_trainer(self, TrainerClass, model_name, loaders, trainer_name=None, experiment_name=None, extra=None):
        return TrainerClass(
            model_name,
            loaders,
            experiment_name=experiment_name,
            trainer_name=self.a.with_suffix(trainer_name or model_name),
            log_dir=self.a.log_dir,
            lr=self.a.lr,
