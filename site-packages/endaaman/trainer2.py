...
class BaseTrainer(metaclass=ABCMeta):
            ...
            with open(config_file_path, 'r', encoding='utf-8') as f:
                ...
                if past_config == self.config:
                    ...
                    if os.path.exists(chp_path):
                        ...
                        print(f'Automatically restored from {chp_path}')
                    else:
                        print(f'Old checkpoint {chp_path} does not exist. Starting from the begining.')
                else:
                    m = f'The name {self.out_dir} already exists, and differenct config. \n' \
                        f'current config:\n{self.config}\n' \
                        f'past config:\n{past_config}\n'
                    print(m)
                    if not yes_no_input('Overwrote this?'):
                        sys.exit(1)
    ...
    def is_achieved_best(self):
        ...
        return np.argmin(h['loss']) == len(h) - 1
