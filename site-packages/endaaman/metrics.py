...
class MetricsFn:
    def __init__(self, fmt='{:.4f}', threshold=0.5, use_mean=False, group=None):
        self.fmt = fmt
...
class AccuracyByChannel(MetricsFn):
    ...
    def calc(self, preds, gts):
        target_idx = gts == target_channel
        preds = preds[target_idx]
        gts = gts[target_idx]

        preds_label = np.argmax(preds, axis=1)
        if self.by_index:
            gts_label = labels.cpu()
        else:
            gts_label = torch.argmax(labels.cpu(), axis=1)
        correct = torch.sum(gts_label == pred_label)
        return correct / len(gts_label)
