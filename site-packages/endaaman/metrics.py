...
class MetricsFn:
    def __init__(self, format='.3f', threshold=0.5, use_mean=False, group=None, selector=None):
        self.format = format
        self.threshold = threshold
        self.use_mean = use_mean
        self.group = group
        self.selector = selector or (lambda preds, gts: (preds, gts))

    def __call__(self, preds, gts):
        preds, gts = self.selector(preds, gts)
        return self.calc(preds, gts)
...
class MultiMacroAccuracy(MetricsFn):
    def __init__(self, by_index=True, **kwargs):
        super().__init__(**kwargs)
        self.by_index = by_index
