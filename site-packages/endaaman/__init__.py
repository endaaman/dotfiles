...
class Commander():
    ...
    def __init__(self, obj=None, with_torch=False):
        self.obj = self
        ...
        if self.with_torch:
            ...
            self._common_parser.add_argument('--seed', type=int, default=42)
        self.try_exec('arg_common', [self._common_parser], warn=False)
        subparsers = self._parser.add_subparsers()

        targets = [k[4:] for k in dir(self) if re.match(r'^run_.+', k)]
        for target in targets:
            target_as_param = re.sub('_', '-', target) # "_" -> "-"
            subparser = subparsers.add_parser(target_as_param, parents=[self._common_parser])
    ...
    def run(self, manul_args=None):
        ...
        target_as_param = self.args.target
        ...
        target = re.sub('-', '_', target_as_param) # "-" -> "_"
        self.try_exec(f'pre_{target}', [], warn=False)

        func_name = f'run_{target}'
...
class TorchCommander(Commander):
    def __init__(self):
        super().__init__(with_torch=True)
