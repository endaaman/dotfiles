...
class Commander():
    ...
    def __init__(self, is_trainer=False):
        ...
        if is_trainer:
            self.use_gpu = not self._args.cpu and torch.cuda.is_available()
    ...
    def run(self):
        target = 'default'
        if hasattr(self._args, 'target'):
            # parser.print_help()
            # exit(0)
            target = self._args.target

        self.try_exec('pre_' + target, [], warn=False)
        print('Starting...')
        args_names = vars(self._args).keys()
        if len(args_names) > 0:
            maxlen = max([len(k) for k in args_names])
            items = vars(self._args).items()
            if self.is_trainer:
                items = [('mode', self.mode), *items]
            for k, v in items:
                print(f'\t{k.ljust(maxlen)} : {v}')

        self.try_exec('run_' + target, [self._args], warn=True)
        print('Done.')
